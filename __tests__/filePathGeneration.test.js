...
describe("full file path generation", () => {
	it("should change paths to absolute", () => {
		const paths = PurgeSvg.globPaths([`${root}paths/index.html`, `${root}paths/index-2.html`]);
	...
	it("should change paths to absolute and find files by pattern", () => {
		const paths = PurgeSvg.globPaths([`${root}paths/**/*.html`, `${root}paths/index.php`]);
	...
	it("should remove duplicate path keys", () => {
		const paths = PurgeSvg.globPaths([`${root}paths/**/*.html`, `${root}paths/index.html`]);
